...
A2A_BATCH_SIZE="${A2A_BATCH_SIZE:-8}"
A2A_ACCUMULATION_STEPS="${A2A_ACCUMULATION_STEPS:-4}"
...
run_stage() {
    ...
    printf '\n=== %s: %s, %s epoch(s), %s, batch=%s, grad_accum=%s ===\n' \
        "$save_weight" "$data_file" "$epochs" "$mode" "$batch_size" "$accumulation_steps"
    if [[ "${DRY_RUN:-0}" == 1 ]]; then
        printf '[dry-run]'
        printf ' %q' "${command[@]}"
        printf '\n'
...
run_stage sft_full_t2a llm sft_t2a.parquet 6 5e-4 all 1 512 "$BATCH_SIZE" "$ACCUMULATION_STEPS"
run_stage sft_full_a2a_proj sft_full_t2a sft_a2a.parquet 1 5e-4 audio_proj 0 1024 "$BATCH_SIZE" "$ACCUMULATION_STEPS"
run_stage sft_full_a2a sft_full_a2a_proj sft_a2a.parquet 3 5e-5 all 0 1024 "$A2A_BATCH_SIZE" "$A2A_ACCUMULATION_STEPS"
run_stage sft_full_i2t_proj sft_full_a2a sft_i2t.parquet 1 5e-5 vision_proj 1 768 "$BATCH_SIZE" "$ACCUMULATION_STEPS"
run_stage sft_full_i2t sft_full_i2t_proj sft_i2t.parquet 1 5e-6 all 1 768 "$BATCH_SIZE" "$ACCUMULATION_STEPS"
run_stage sft_full_a2a_final sft_full_i2t sft_a2a.parquet 1 5e-6 all 0 1024 "$A2A_BATCH_SIZE" "$A2A_ACCUMULATION_STEPS"
run_stage sft_omni sft_full_a2a_final sft_i2t.parquet 1 5e-6 vision_proj 1 768 "$BATCH_SIZE" "$ACCUMULATION_STEPS"
