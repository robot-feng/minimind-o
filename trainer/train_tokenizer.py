...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Train a MiniMind-compatible ByteLevel BPE tokenizer")
    parser.add_argument("--data_path", default=DATA_PATH)
    parser.add_argument("--tokenizer_dir", default=TOKENIZER_DIR)
    ...
    parser.add_argument("--max_lines", type=int, default=MAX_LINES,
                        help="Limit input rows for a quick experiment; 0 reads all rows")
    parser.add_argument("--skip_eval", action="store_true")
    args = parser.parse_args()
    if args.vocab_size <= args.special_tokens_num or args.max_lines < 0:
        parser.error("vocab_size must exceed special_tokens_num and max_lines must be nonnegative")
    train_tokenizer(args.data_path, args.tokenizer_dir, args.vocab_size,
                    args.special_tokens_num, args.max_lines)
